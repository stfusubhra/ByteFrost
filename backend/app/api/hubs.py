...
async def create_hub(hub_in: HubCreate, db: AsyncSession = Depends(get_db)):
...
async def get_hubs(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Hub).offset(skip).limit(limit))
    return result.scalars().all()


@router.get("/{hub_id}", response_model=HubResponse)
async def get_hub(hub_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def update_hub(hub_id: UUID, hub_in: HubUpdate, db: AsyncSession = Depends(get_db)):
    hub = await db.get(Hub, hub_id)
    if not hub:
        raise HTTPException(status_code=404, detail="Hub not found")
